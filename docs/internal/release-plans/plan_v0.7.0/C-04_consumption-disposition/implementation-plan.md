...
this effort ships in v0.7.0, after three intervening releases (v0.4.0 through v0.6.0) will already have
...
states it produces no files, confirmed in Phase 2). Plugin 0.7.0.
...
- [ ] Step 4: Bump both `plugin.json` manifests and `library.json` to 0.7.0, then regenerate
      `manifest.generated.json` from `library.json`.
- [ ] Step 5: Write the `HISTORY.md` entries and the `CHANGELOG.md` `[0.7.0]` section describing the
      new field.
...
.codex-plugin/plugin.json library.json` shows `0.7.0` for the plugin and `1.9.0` / `1.8.0` for the two
components.
...
repository's conformance gate (CI-01, v0.3.0) already covers or should cover; this effort is verified
