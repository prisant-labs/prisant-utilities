...
version/documentation bookkeeping this release requires. Because this effort ships in v0.7.0, after
three intervening releases (v0.4.0 through v0.6.0) will already have changed the files it touches,
...
**Steps:**
- [ ] Step 1: Read the shipped v0.3.0 state of D-07's carry-forward step in `SKILL.md` and decide
...
raising log's filename), and the current-repository default per `plan_v0.7.0.md`'s open D2.

**Files:**
- Modify: `skills/plab-wrap-session/SKILL.md` (issue content template; current-repository default
  statement; a one-line pointer to `plan_v0.7.0.md`'s D2 for future revision)
...
**Goal:** Ship the version and documentation bookkeeping this release requires, per `plan_v0.7.0.md`'s
...
- [ ] Step 3: Bump both `plugin.json` manifests and `library.json` to 0.7.0, then regenerate
      `manifest.generated.json` from `library.json`.
- [ ] Step 4: Write the `HISTORY.md` entries and the `CHANGELOG.md` `[0.7.0]` section describing the
      promotion offer.
...
library.json` shows `0.7.0` for the plugin and `1.9.0` / `1.8.0` for the two components.
...
No CI change from this effort. The repository's CI, established by CI-01 in v0.3.0, verifies
...
- The mechanical version-bump rows across `.claude-plugin/plugin.json`, `.codex-plugin/plugin.json`,
  `library.json`, and `manifest.generated.json` are `plan_v0.7.0.md`'s Doc-Update Checklist rows; this
