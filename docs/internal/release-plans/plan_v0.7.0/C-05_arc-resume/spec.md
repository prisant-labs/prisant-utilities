...
- **Not cold-repo re-orientation.** C-03 (the roadmap's C-3, cold-repo degradation, a separate effort scheduled at v0.5.0, before this one) handles the case of no recent log or no log at all. C-05 assumes multiple logs exist; it does not build a fallback for when they do not.
