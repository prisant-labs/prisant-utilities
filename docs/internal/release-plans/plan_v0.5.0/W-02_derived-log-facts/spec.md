...
5. Does not change the plugin-level version in `.claude-plugin/plugin.json` or `.codex-plugin/plugin.json` beyond what the existing generator produces from `library.json`. The release-level `0.5.0` plugin version is the release plan's own gate to set, not a step in this effort's plan.
