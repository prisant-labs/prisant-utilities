...
tracked in `docs/internal/release-plans/plan_v0.3.0/plan_v0.3.0.md`'s Doc-Update Checklist, shared
across every v0.3.0 effort that touches wrap. Do not bump `metadata.version` in this plan's phases.
...
- **Sequencing:** this plan must land, and be merged to `main`, before D-11 (three-state gate canaries)
  (`docs/internal/release-plans/plan_v0.3.0/D-11_three-state-gate-canaries/`) begins its own Phase 1. D-11
...
- [ ] Step 2: Check `skills/plab-wrap-session/HISTORY.md` for an existing `## 1.6.0` heading and
  version-table row. If a co-landing v0.3.0 effort has already added one, append this effort's content
  ...
  <date> | v0.3.0 | fixed | <short summary, to be extended by later v0.3.0 efforts> |`) and the
...
v0.3.0 efforts have appended bullets beneath it.
...
- `skills/plab-wrap-session/HISTORY.md`: new bullet under the shared `## 1.6.0` heading (Phase 2, Steps
  2-3), co-owned with the other v0.3.0 efforts that touch wrap.
...
- Root `README.md`, `.claude-plugin/plugin.json`, `.codex-plugin/plugin.json`, `library.json`,
  ...
  v0.3.0 effort that bumps wrap to 1.6.0. Bumping them once per effort would produce competing edits to
