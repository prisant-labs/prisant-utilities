...
at 0.2.0 through this effort; that bump is a release-level action gated by all eight v0.3.0 efforts
completing (`plan_v0.3.0.md` Hygiene Gate (f)).
...
- If the shared `## 1.6.0` heading in `skills/plab-wrap-session/HISTORY.md` already exists (a sibling
  v0.3.0 effort landed first), append this effort's own paragraph under it rather than duplicating the
  heading or row.
...
- [ ] Step 3: Bump wrap's frontmatter:
  ...
  Skip this step if a sibling v0.3.0 effort (D-04 or D-05) has already bumped it to 1.6.0 this
  release.
...
not add or move any `.github/workflows/` file. CI-01 (a separate v0.3.0 effort) gives this repository
...
- `library.json`: `plab-wrap-session` component version only (Phase 2). The top-level plugin version,
  ...
  `plan_v0.3.0.md` Hygiene Gate (f), that bump happens once, at tag time, after all eight v0.3.0
