...
**Skill versions.** `plab-wrap-session` 1.6.0, `plab-continue-session` 1.4.0, plugin 0.3.0. Both
...
| Doc | Update | Done |
|-----|--------|------|
| `CHANGELOG.md` | Move items from [Unreleased] to v0.3.0 section | [ ] |
...
| `.claude-plugin/plugin.json` | Bump plugin `version` to 0.3.0 | [ ] |
...
| Git tag `v0.3.0` | Create the annotated tag once all of the above are done | [ ] |
...
|----|-------|------------|--------|---------|
| D1 | Batch as one release or split | Ship as one v0.3.0 | Proposed | 2026-08-23 |
...
**Context.** An earlier draft of this plan split them: gates and CI in v0.3.0, the log-corpus defects
in v0.4.0. That contradicted two existing records. The 2026-08-18 addendum's own batching table
...
* **Option A:** One v0.3.0 with all eight efforts. Larger release, single pair bump, matches the
  addendum.
* **Option B:** Split into v0.3.0 (gates, CI) and v0.4.0 (log corpus). Smaller units, but two pair
...
> * **Choice (proposed):** Option A, one v0.3.0 containing all eight efforts.
