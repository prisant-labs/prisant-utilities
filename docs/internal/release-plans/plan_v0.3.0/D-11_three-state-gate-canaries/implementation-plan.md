...
tracked in `docs/internal/release-plans/plan_v0.3.0/plan_v0.3.0.md`'s Doc-Update Checklist, shared across
every v0.3.0 effort that touches wrap. Do not bump `metadata.version` in this plan's phases.
...
- **Sequencing precondition:** D-12 (path citation precision)
  (`docs/internal/release-plans/plan_v0.3.0/D-12_path-citation-precision/`) must already be merged to
...
- [ ] Step 1: Create the file with a module docstring stating the exit-code contract (same 0/1/2 shape
  ...
  `docs/internal/release-plans/plan_v0.3.0/D-12_path-citation-precision/spec.md` rather than restating
...
- [ ] Step 1: Re-open `skills/plab-wrap-session/SKILL.md` and re-verify current line numbers for the Log
  ...
  wording, and other v0.3.0 efforts landing before this one may have shifted line numbers elsewhere in
  the file).
...
- [ ] Step 2: Check `skills/plab-wrap-session/HISTORY.md` for an existing `## 1.6.0` heading (D-12, or
  another co-landing v0.3.0 effort, may have already created it). Append this effort's bullet(s) under
...
heading and one matching version-table row regardless of how many v0.3.0 efforts have appended bullets
beneath it.
...
using the same canary-before-trust pattern (`docs/internal/release-plans/plan_v0.3.0/plan_v0.3.0.md`,
...
- `skills/plab-wrap-session/HISTORY.md`: new bullet(s) under the shared `## 1.6.0` heading (Phase 4,
  Step 2), co-owned with D-12 and the other v0.3.0 efforts that touch wrap.
...
- Root `README.md`, both plugin manifests, `library.json`, `manifest.generated.json`, and the
  ...
  `docs/internal/release-plans/plan_v0.3.0/plan_v0.3.0.md`'s Doc-Update Checklist, shared across every
  v0.3.0 effort that bumps wrap to 1.6.0.
