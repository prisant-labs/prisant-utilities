...
- Modify: `library.json` (bump the `plab-continue-session` component version to 1.7.0; confirm the top-level plugin version is 0.6.0, already bumped by W-04's implementation plan if it ran first)
- Modify: `manifest.generated.json` (regenerate; do not hand-edit, per `AGENTS.md`)
- Modify: `.claude-plugin/plugin.json` and `.codex-plugin/plugin.json` (confirm `version` reads `0.6.0` in both; only bump here if W-04's plan has not already done so)
...
- Modify: `CHANGELOG.md` (add to the same `[Unreleased]` or `[0.6.0]` entry W-04 added to, Added: `--arc [N]` flag)
...
- [ ] Step 2: Bump `library.json`'s `plab-continue-session` entry; confirm rather than re-bump the top-level plugin version and both native manifests if W-04's plan already set them to 0.6.0 in this same release.
