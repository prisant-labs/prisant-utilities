...
version/documentation bookkeeping this release requires. Because this effort ships in v0.9.0, after
three intervening releases (v0.6.0 through v0.8.0) will already have changed the files it touches,
...
raising log's filename), and the current-repository default per `plan.md`'s open D2.

**Files:**
- Modify: `skills/plab-wrap-session/SKILL.md` (issue content template; current-repository default
  statement; a one-line pointer to `plan.md`'s D2 for future revision)
...
**Goal:** Ship the version and documentation bookkeeping this release requires, per `plan.md`'s
...
- The mechanical version-bump rows across `.claude-plugin/plugin.json`, `.codex-plugin/plugin.json`,
  `library.json`, and `manifest.generated.json` are `plan.md`'s Doc-Update Checklist rows; this
